  ...
  useEffect(() => {
    const loadGame = async () => {
      ...
      const { data: playersData } = await supabase
        .from('game_players')
        .select(`id, user_id, role, is_alive, joined_at, guest_name, guest_phone, users(username)`)
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-600 to-purple-600 p-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <Link href="/dashboard" className="text-white hover:text-red-100 font-semibold">
            ← Back
          </Link>
        ...
        {/* Players Card */}
        <div className="bg-white rounded-lg shadow-xl p-8 mb-6">
          <h3 className="text-2xl font-bold text-gray-800 mb-4">Players ({players.length})</h3>
          ...
          <div className="bg-white rounded-lg shadow-xl p-8">
            <div className="mb-6">
              <label className="block text-gray-700 font-semibold mb-3">
                Max Rounds: <span className="text-2xl text-red-600">{maxRounds}</span>
              </label>
              <input
                type="range"
                min="1"
            ...
            <button
              ...
              className="w-full bg-red-600 hover:bg-red-700 disabled:bg-gray-400 text-white font-bold py-3 rounded-lg transition text-lg"
            >
              {players.length < 4
                ? `Start Game (Need ${4 - players.length} more players)`
                : 'Start Game'}
            </button>
          </div>
        )}
