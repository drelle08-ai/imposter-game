...
export default function GuestGamePage() {
  ...
  useEffect(() => {
    const loadGame = async () => {
      ...
      const currentGuest = (playersData as any)?.find(
        (p: any) => p.guest_phone === guestPhone
      );
      setGuestPlayer(currentGuest || null);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md text-center">
          ...
          <p className="text-sm text-gray-500">Thanks for playing!</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {guestPlayer && (
          ...
          >
            ...
            <p className="text-3xl font-bold capitalize">{guestPlayer.role}</p>
          </div>
        )}
        ...
        {currentRound?.phase === 'voting' && (
          <div className="bg-white rounded-lg shadow-xl p-6 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {players
                .filter((p) => p.is_alive && p.guest_phone !== guestPhone)
                .map((player) => (
                  <button
                    key={player.id}
                    ...
                    className={`p-4 rounded-lg font-semibold transition ${
                      guestPlayer?.voted_for_user_id === player.id
            ...
            {players.map((player) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  >
                    {player.guest_name || player.users?.username}
                    {player.guest_phone === guestPhone && ' (You)'}
                  </span>
